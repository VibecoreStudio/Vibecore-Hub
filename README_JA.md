...
> **言語について：** v1.2.1 のアプリ画面は現在、簡体字中国語のみです。日本語 UI は今後のバージョンで対応予定です。

## すぐに使い始める

> [!IMPORTANT]
> ## ほぼすべての機能と項目で右クリックが使えます
> **上部のモジュールアイコンを右クリック：その機能の設定を開きます。**
>
> **ショートカット、メモ、フォルダー、音声プリセットを右クリック：編集、削除、その他の操作を表示します。**
>
> 右クリックを覚えれば、Vibecore Hub の基本操作はほぼ完了です。

- Vibecore ロゴ部分をドラッグしてウィンドウを移動し、右クリックで全体設定を開きます
- 左右の枠をドラッグして幅を、各モジュール下部の `···` をドラッグして高さを調整します
- アプリ、ファイル、フォルダー、Windows ショートカットをショートカットライブラリへドロップすると自動登録されます
- ショートカット、メモ、フォルダーカードはドラッグで並べ替えられ、明るい挿入線が移動先を示します
- ピンで最前面表示を切り替え、`×` でシステムトレイへ格納します

詳しい操作は [Vibecore Hub ユーザーガイド](docs/USER_GUIDE_JA.md) をご覧ください。

## 多言語対応ロードマップ

v1.2.1 のアプリ画面は現在、簡体字中国語のみです。**次のバージョンではアプリ内に言語メニューを追加し、簡体字中国語、English、日本語をワンクリックで切り替えられるようにします。**
